...
export class IdiomaController {
  ...
  async count(
    ...
    return await SqlFilterUtil.ejecutarQueryCount(dataSource, 'idioma', where);
  }
  ...
  async find(
    ...
    const dataSource = this.idiomaRepository.dataSource;
    const camposSelect = "*"
    return await SqlFilterUtil.ejecutarQuerySelect(dataSource, 'idioma', filter, camposSelect);
  }
