...
import {EmpresaSensor, EmpresaSensorRelations, TipoSensor, Empresa} from '../models';
import {TipoSensorRepository} from './tipo_sensor.repository';
import {EmpresaRepository} from './empresa.repository';

export class EmpresaSensorRepository extends DefaultCrudRepository<
  EmpresaSensor,
  typeof EmpresaSensor.prototype.id,
  EmpresaSensorRelations
> {

  public readonly tipoSensor: BelongsToAccessor<TipoSensor, typeof EmpresaSensor.prototype.id>;
  public readonly empresa: BelongsToAccessor<Empresa, typeof EmpresaSensor.prototype.id>;
  ...
  constructor(
    ...
    super(EmpresaSensor, dataSource);
   ...
   */
  async findByEmpresa(empresaId: number): Promise<EmpresaSensor[]> {
    return this.find({
      where: {
        empresaId: empresaId
      },
   ...
   */
  async findByTipoSensor(tipoSensorId: number): Promise<EmpresaSensor[]> {
    return this.find({
      where: {
   ...
   */
  async findByEmpresaAndTipo(empresaId: number, tipoSensorId: number): Promise<EmpresaSensor | null> {
    return this.findOne({
      where: {
        and: [
          {empresaId: empresaId},
